...
- **packages/db** — Drizzle schema (`src/schema/`: `users`, `sessions` + `user_role` pgEnum), drizzle-zod select schemas (omit secrets like `passwordHash` from public shapes), Postgres client, and `runMigrations()` (called on API boot in `apps/api/src/index.ts`). IDs stay nanoid text — do not switch to identity columns.
