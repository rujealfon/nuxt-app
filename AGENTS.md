...
- **apps/app** — Nuxt 4 SPA (`ssr: false`) for the authenticated product. Extends `layer-auth` + `layer-base`. On Vercel, Nitro `vercel-static` (CDN rewrites for SPA deep links and `/__api`); locally/Compose, `node-server`.
- **apps/admin** — Nuxt 4 SPA (`ssr: false`) for admins. Extends `layer-auth` + `layer-base`. Same Vercel/local Nitro split as app.
...
- **layers/base** (`@nuxt-app/layer-base`) — Tailwind v4 + `@nuxt/ui` + shared Nitro config (`vercel-static` when `VERCEL` is set, `node-server` otherwise). Every Nuxt app depends on this. `app.vue` wraps pages in `UApp`. An SSR app should override `nitro.preset` to `vercel`.
